...
class LineNumberRulerView: NSRulerView {
    ...
    override func drawHashMarksAndLabelsInRect(rect: NSRect) {
        ...
        if let context = NSGraphicsContext.currentContext()?.CGContext {
            ...
            CTFontGetAdvancesForGlyphs(ctFont, CTFontOrientation.Horizontal, &digitGlyphs[8], &advance, 1)
            ...
            lineNum += regex!.numberOfMatchesInString(text, options: NSMatchingOptions(), range: NSMakeRange(0, characterCount))
            
            
            var glyphIndex: Int
            for glyphIndex = glyphCount; glyphIndex < NSMaxRange(visibleGlyphRange); lineIndex += 1 {
